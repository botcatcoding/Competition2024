...
public class RobotContainer {
    ...
    AutoManager autoCommand = new AutoManager(this, lighting, m_robotDrive, slurp, shooter, elbow, shoulder,
            kinamatics,
            slurpDetect);
    private final Diagnostics diagnosics = new Diagnostics(m_robotDrive, elbow, shooter, shoulder, slurp, autoCommand);
    ...
    public RobotContainer() {
        kinamatics = new Kinamatics(m_robotDrive);
    ...
    private void configureButtonBindings() {
        ...
        new JoystickButton(m_mechDriver, 6)
                .whileTrue(new ShootToThrill(m_robotDrive, lighting, slurp, shooter, elbow, shoulder, kinamatics,
                        slurpDetect, false, false, false));
        new JoystickButton(m_mechDriver, 5)
                .whileTrue(new AmpScoreCommandGroupWrapper(m_robotDrive, slurp, elbow, shoulder, slurpDetect));
        ...
        new JoystickButton(m_driverController, 10)
                .whileTrue(new ShutUpAndDanceCmdGroup(lighting, m_robotDrive, elbow, shoulder, slurpDetect, slurp,
                        shooter));
        ...
        shouldClimbUp.castTo(Trigger::new)
                .whileTrue(new ConditionalCommand(new ClimberCommands.ClimbUpGroup(shoulder, elbow, false),
                        new WaitCommand(0),
                        m_mechDriver.button(1, CommandScheduler.getInstance().getActiveButtonLoop())))
                .whileFalse(co);
        shouldClimbDown.castTo(Trigger::new)
                .whileTrue(new ConditionalCommand(new ClimberCommands.AintNoMountainHighEnough(shoulder, elbow, false),
                        new WaitCommand(0),
                        m_mechDriver.button(1, CommandScheduler.getInstance().getActiveButtonLoop())))
                .whileFalse(co);
        // // POV
        new POVButton(m_mechDriver, 0)
                .whileTrue(new MoveToSlurpPositionWrapper(lighting, shoulder, elbow, shooter, m_robotDrive,
